...
class QLearningOneStepPlayer(Game.Player):
    def __init__(self, print_plays=False, save_plays=False, name="QLearningOneStep", lr=0.1, mode="QLearning1Step"):
    ...
    def reward(self, reward, split_reward):
        """
        :param: reward: reward for the first hand in case of split
        :param: split_reward: reward for the second hand in case of split
        :return: None
        """
        ...
        for i, (state, action) in enumerate(self.memory):
            if not split:
                # Update the value in the dictionary with (1-lr)*current_value + lr*reward
    ...
    def update_file(self):
        """
        Update the player memory, so we can continue the training/game later
        :return:
        """
